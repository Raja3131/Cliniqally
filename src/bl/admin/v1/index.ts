...
export async function signUpAdmin(req: any, res: any) {
    try {
        if (req?.body?.email && req?.body?.password && req?.body?.name) {
            const emailResult = await vaasPgQuery(`SELECT * FROM admins WHERE email = '${req?.body?.email}'`, [], cachingType.StandardCache)
            if (emailResult?.queryResponse.length === 0) {
                const adminId = uuidv4()
                bcrypt.hash(req.body.password, 10, async (error, hash) => {
                    if (!error) {
                        req.body.password = hash;
                        const createAdminUser = await vaasPgQuery(`INSERT INTO admins (admin_id,name,email,password,user_type,country_code,mobile,status,created_by,updated_by) VALUES ('${adminId}','${req?.body?.name}', '${req?.body?.email}','${req.body.password}','admin','${req?.body?.country_code}','${req?.body?.mobile}','enabled','${adminId}','${adminId}')`, [], cachingType.StandardCache)
                        if (createAdminUser) {
                            res.status(200).json(
                                {
                                    status: true,
